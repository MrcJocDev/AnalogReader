...
def read_gauge(image_path):
    ...
    x4 = int(x +(length_n * math.cos(90)))
    y4 = int(y + (length_n * math.sin(90)))
    ...
    min_value = -1  # Example minimum value
    max_value = 3  # Example maximum value
    ...
    cv2.line(image, (x, y), (x4, y4), (0, 0, 255), 2)

    # Draw the results
    plt.figure(figsize=(10, 10))
    plt.imshow(cv2.cvtColor(image, cv2.COLOR_BGR2RGB))
    plt.title(f'Reading: {angle_cp:.2f}')
    plt.show()
